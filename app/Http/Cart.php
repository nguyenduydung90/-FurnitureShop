...
class Cart
{
    ...
    public function add($item, $id)
    {
        ...
        $storedItem = ['qty' => 0, 'item' => $item, 'price' => 0];
        if ($this->items) {
        ...
        $storedItem['qty']++;
        $storedItem['price'] = $item->buyPrice * $storedItem['qty'];
        $this->items[$id] = $storedItem;

        //Tang tong so luong san pham trong gio hang
        $this->totalQty++;
        //Tinh tong gia tien trong gio hang
        $this->totalPrice = $this->totalPrice + $item->buyPrice;
    }
    ...
    public function update( $request, $id)
    {
        if ($this->items) {
            ...
            if (array_key_exists($id, $itemsIntoCart)) {
                ...
                $priceUpdate = $itemUpdate['item']->buyPrice * $request->qty - $itemUpdate['price'];
